...
def extract_poses_from_csv(input_csv, output_csv):
    ...
    for _, row in df.iterrows():
        pose_data.append({
            ...
            'position.x': row["pose.pose.position.x"],
            'position.y': row["pose.pose.position.y"],
            'position.z': row["pose.pose.position.z"],
            'orientation.x': row["pose.pose.orientation.x"],
            'orientation.y': row["pose.pose.orientation.y"],
            'orientation.z': row["pose.pose.orientation.z"],
            'orientation.w': row["pose.pose.orientation.w"],
        })
...
def calculate_trajectory_errors(bag_folder, run_id, topics):
    traj_topic_name = topics.get('trajectory_plan', {}).get('name')
    est_topic_name = topics.get('estimated_position', {}).get('name')

    if not traj_topic_name or not est_topic_name:
        print("[ERROR] Could not find real or estimated position topics in config.")
        return None

    run_folder = os.path.join(bag_folder, "csv_files", "per_run", f"run_{run_id}")
    traj_path = os.path.join(run_folder, f"{traj_topic_name}.csv")
    est_path = os.path.join(run_folder, f"{est_topic_name}.csv")

    if not os.path.isfile(traj_path):
        print(f"[ERROR] Ground truth file not found: {traj_path}")
        return None
    if not os.path.isfile(est_path):
        print(f"[ERROR] Estimated position file not found: {est_path}")
        return None
    ...
    # --- Trajectory Error ---
    traj_pos = extract_position_columns(traj_df, label="Planned trajectory")
    est_pos = extract_position_columns(est_df, label="Estimated Position")
    
    # Extract position arrays
    traj_points = traj_pos.to_numpy()
    est_points = est_pos.to_numpy()

    errors = []
    for ref_pt in traj_points:
        distances = np.linalg.norm(est_points - ref_pt, axis=1)
    ...
    # --- Print and Return All ---
    print(f"[INFO] Trajectory RMSE (run {run_id}): {rmse_pos}")
...
def calculate_and_save_all_errors(bag_folder, run_ids, topics, position_error=True, yaw_error=True, velocity_error=True, trajectory_error=True):
    all_position_errors = []
    all_velocity_errors = []
    ...
    for run_id in run_ids:
        ...
        # --- Trajectory Errors ---
        if trajectory_error:
            traj_errors = calculate_trajectory_errors(bag_folder, run_id, topics)

            if traj_errors:
                flat_traj = {'run': run_id}
                for category, subdict in traj_errors.items():
                    for error_type, val_dict in subdict.items():
                        key = f"{category}_{error_type}"
                        flat_traj[key] = val_dict
                all_trajectory_errors.append(flat_traj)
                save_errors_to_csv(traj_errors, bag_folder, run_id, label="trajectory")
    ...
    if all_trajectory_errors:
        df_vel = pd.DataFrame(all_trajectory_errors)
        df_vel.to_csv(os.path.join(errors_folder, "all_runs_trajectory_errors.csv"), index=False)
        print("[INFO] Saved trajectory errors for all runs.")
